...
export function HomePage() {
  ...
  // Add file preview panel to actions sidebar
  if (fileData && fileId && user?.id) {
    actions.push(
      <ActionSidebarItem
        title="File Preview"
        tooltipText="Overview of file statistics and column types"
      >
        <FilePreviewPanel userId={user.id} fileId={fileId} />
      </ActionSidebarItem>,
    );
  }
  ...
  return (
    <FileLayout actions={actions}>
      <div className="w-full h-full flex flex-col gap-4">
        ...
        {hasPendingEdits && (
          <Alert className="flex-shrink-0 mx-6">
        ...
        {fileData && !isLoadingData && !dataError && (
          <div className="flex-1 min-h-0 px-6">
            <Tabs defaultValue="data" className="flex flex-col h-[calc(100vh-200px)]">
              <TabsList className="flex-shrink-0">
                <TabsTrigger value="data">Show Data</TabsTrigger>
                <TabsTrigger value="summary">Data Summary</TabsTrigger>
              </TabsList>
              <TabsContent value="data" className="mt-4 border rounded-lg flex-1 min-h-0 overflow-hidden">
                <DataTable columns={fileData.columns} rows={fileData.rows} />
              </TabsContent>
              <TabsContent value="summary" className="mt-4 border rounded-lg flex-1 min-h-0 overflow-hidden">
                <DataSummary columns={fileData.columns} rows={fileData.rows} />
              </TabsContent>
            </Tabs>
          </div>
        )}
