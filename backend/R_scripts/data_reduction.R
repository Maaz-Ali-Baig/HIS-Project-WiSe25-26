...
reduce_data_csv <- function(
    ...
    for (col_name in names(data)) {
        ...
        if (numeric_cols[[col_name]]) {
            ...
            unique_count <- length(unique(vec_num[!is.na(vec_num)]))
    ...
    fit_data <- data_clean
    sample_size_used <- nrow(data_clean)
    seed_used <- NULL
    ...
    if (!is.null(dr_table_path)) {
        utils::write.csv(coords_df, dr_table_path, row.names = FALSE, na = "")
    }
